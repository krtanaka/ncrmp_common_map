...
sets_i = sets_i[,c("SITE_ID", "SITE_NO", "X", "Y", "LONGITUDE", "LATITUDE", "DEPTH_BIN", "DEPTH", "REEF_ID")]

sets_i <- sets_i %>% 
  mutate_if(is.numeric, round, digits = 4) %>% 
  mutate_if(is.character, toupper)

sets_i$DEPTH = round(sets_i$DEPTH, 2)
...
for (d in 1:length(map_direction)) {
  ...
  if (map_direction[d] == "NW") ext = c(min(sets$longitude, na.rm = T) - space, mean(sets$longitude, na.rm = T) + space, median(sets$latitude, na.rm = T) - space, max(sets$latitude, na.rm = T) + space)
  if (map_direction[d] == "NE") ext = c(mean(sets$longitude, na.rm = T) - space, max(sets$longitude, na.rm = T) + space, median(sets$latitude, na.rm = T) - space, max(sets$latitude, na.rm = T) + space)
  if (map_direction[d] == "SW") ext = c(min(sets$longitude, na.rm = T) - space, mean(sets$longitude, na.rm = T) + space, min(sets$latitude, na.rm = T) - space, mean(sets$latitude, na.rm = T) + space)
  if (map_direction[d] == "SE") ext = c(mean(sets$longitude, na.rm = T) - space, max(sets$longitude, na.rm = T) + space, min(sets$latitude, na.rm = T) - space, mean(sets$latitude, na.rm = T) + space)
  ...
  map_i = 
    ...
    geom_spatial_point(data = sets_i, aes(longitude, latitude, fill = depth_bin), shape = 21, size = 3, crs = 4326) +
    # geom_spatial_point(data = cells, aes(longitude, latitude), color = "white", alpha = 0.8, shape = 22, size = 0.01, crs = 4326) +
    ...
    scale_x_continuous(sec.axis = dup_axis(), "", limits = ext[1:2], expand = c(-0.005, 0.005)) +
    scale_y_continuous(sec.axis = dup_axis(), "", limits = ext[3:4], expand = c(-0.005, 0.005))
  ...
  pdf(paste0("outputs/maps/survey_map_", region, "_", islands[i], "_", map_direction[d], ".pdf"), height = 10, width = 18)
