...
b8 <- list.files("N:/GIS/Projects/CommonMaps/Bathymetry/2022_rainier_rota/", pattern = "\\.tif$", full.names = TRUE)
b8 <- lapply(b8, rast)
template <- b8[[4]] # Resample all rasters to match the resolution and extent of the 5th raster, Use the 5th raster as the template
b8 <- lapply(b8, function(r) resample(r, template, method = "near"))
b8 <- do.call(merge, b8)
b8[b8 >= 0] <- NA
b8[b8 <= -30] <- NA
plot(b8); res(b8)
...
fine_topo <- mean(c(b1, b2, b3, b4, b5, b7, b8), na.rm = TRUE)
