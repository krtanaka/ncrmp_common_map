...
fine_bathymetry_files <- list(
  b1 = "N:/GIS/Projects/CommonMaps/Bathymetry/tinian_5m.asc",
  b2 = "N:/GIS/Projects/CommonMaps/Bathymetry/tinlidballmos.asc",
  b3 = "N:/GIS/Projects/CommonMaps/Bathymetry/tinmblidbmos.asc"
)
...
# -----------------------------
# 2. Resample Rasters to Match `b3`
# -----------------------------

# Identify the target raster (`b3`) for resampling
target_raster <- fine_rasters[["b3"]]

# Resample `b1`, `b3`, and `b4` to match `b2` using nearest neighbor (`"near"`) method
resampled_rasters <- lapply(names(fine_rasters)[names(fine_rasters) != "b3"], function(name) {
...
resampled_rasters <- c(resampled_rasters, list(b3 = target_raster))
...
save(topo_i, file = 'data/gis_bathymetry/tin_merged.RData')
