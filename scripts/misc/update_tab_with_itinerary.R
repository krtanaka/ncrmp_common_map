...
for (isl in 1:length(islands)) {
  ...
  tab$reef_short = ifelse(tab$reef_id == "protected_slope", "PRS", tab$reef_short)
  tab$STRATUM = paste0(tab$reef_short, "_", tab$depth_bin_short)
  
  itinerary_i$sector_id <- tolower(substr(itinerary_i$SECTOR, 1, 9))
  
  table(itinerary$SECTOR)
  table(tab$sector_id)
  
  
  tab = left_join(tab, itinerary)
