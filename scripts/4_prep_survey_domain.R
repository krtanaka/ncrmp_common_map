...
load(paste0("data/gis_sector/joy.RData"))
lbsp = raster_and_table[[1]]; lbsp_name = raster_and_table[[2]]
...
lbsp <- resample(lbsp, topo_i, method = "ngb")

rm(topo_i); gc()

# Stack and name layers based on the current island
df <- stack(hardsoft, sector, reef, bathymetry, buffer, lbsp)
names(df) <- c("hardsoft", "sector", "reef", "depth", "buffer", "lbsp")
